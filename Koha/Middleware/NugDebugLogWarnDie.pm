...
our %STAT_CONFIG = (
    ...
    alert_huge_memory_delta => 1024 * 50,
...
# $STATS{log_quits} = undef;
# $STATS{alert_maxmemsize} = 1;
# $STATS{slack_hash} = '';
...
END {
    if ( $STATS{log_quits} ) {
        warn "Child quit at "
          ...
          . ". Lifetime: "
          . sprintf( "%.2f days", ( Time::HiRes::time() - $STATS{child_start} ) / 86400 )
...
sub _send_slack_messages {
    ...
    my $res = $ua->post(
        'https://hooks.slack.com/services/' . $STATS{slack_hash} => { Accept => 'application/json' },
        json                                                     => {
...
sub _make_message {
    ...
    if ( $mode ne 'warn' ) {
        ...
        my $post_params = (
            ...
            ? " Form parameters: " . (
                ...
                : join( ", ", keys %{ $request->body_parameters } )
              )
              . "\n"
            : ''
        );
    ...
    _push_slack_message( $mode, $message ) if $STATS{slack_hash} and !$stats->{ignore_warning};
...
sub call {
    ...
    _send_slack_messages() if $STATS{slack_hash};
    return $res;
}
...
sub print {
    ...
    Koha::Middleware::NugDebugLogWarnDie::_send_slack_messages() if $Koha::Middleware::NugDebugLogWarnDie::STATS{slack_hash};
}
