...
# This test owns an outer DB transaction. Process-owned file sessions keep
# CGI::Session from committing that same handle during session teardown.
...
my $last_disclosure_action_id = 0;

sub actor_disclosure_logs {
...
sub disclosure_logs {
    return actor_disclosure_logs()->search(
        { action_id => { '>' => $last_disclosure_action_id } }
    );
}

sub clear_disclosure_logs {
    my ($latest) = actor_disclosure_logs()->search(
        {}, { order_by => { -desc => 'action_id' }, rows => 1 }
    )->get_column('action_id');
    $last_disclosure_action_id = $latest // 0;
    return;
}
...
subtest 'account-note no-op is audited while a real note change remains deferred' => sub {
    $account_line = Koha::Account::Line->new(
        {
            borrowernumber    => $patron->id,
            amount            => 7,
            amountoutstanding => 7,
...
subtest 'every staff read entrypoint commits before disclosure' => sub {
    my @cases = (
        [ 'circ/circulation.pl',             { borrowernumber => $patron->id }, 'circulation.checkout' ],
        [ 'members/holdshistory.pl',         { borrowernumber => $patron->id }, 'patrons.holds.history' ],
        [ 'members/moremember.pl',           { borrowernumber => $patron->id }, 'patrons.record.details' ],
        [ 'members/notices.pl',              { borrowernumber => $patron->id }, 'patrons.notices.list' ],
        [ 'members/pay.pl',                  { borrowernumber => $patron->id }, 'patrons.account.outstanding' ],
        [ 'members/purchase-suggestions.pl', { borrowernumber => $patron->id }, 'patrons.suggestions.list' ],
        [ 'members/readingrec.pl',           { borrowernumber => $patron->id }, 'patrons.circulation.history' ],
        [ 'members/recallshistory.pl',       { borrowernumber => $patron->id }, 'patrons.recalls.history' ],
        [ 'members/routing-lists.pl',        { borrowernumber => $patron->id }, 'patrons.routing_lists.list' ],
        [ 'members/statistics.pl',           { borrowernumber => $patron->id }, 'patrons.statistics.summary' ],
    );

    for my $case (@cases) {
        my ( $script, $params, $surface ) = @{$case};
        clear_disclosure_logs();
        my $response = request_cgi( $script, 'GET', $params );
        assert_audited_patron_response( $response, $surface, $script );

        clear_disclosure_logs();
        $inject_disclosure_store_failure = 1;
        $response = request_cgi( $script, 'GET', $params );
        $inject_disclosure_store_failure = 0;
        is( $response->code, 503, "$script fails closed when audit storage fails" );
        unlike( $response->as_string, qr/PRIVATE_CGI_PATRON_SENTINEL/, "$script emits no patron data on audit failure" );
        is( disclosure_logs()->count, 0, "$script stores no partial disclosure row" );
    }

    done_testing;
};

subtest 'account line detail includes the displayed manager as a second subject' => sub {
    clear_disclosure_logs();
    my $response = request_cgi(
        'members/accountline-details.pl', 'GET',
        { accountlines_id => $account_line->id }
    );
    is( $response->code, 200, 'the account detail page renders' );
    like( $response->content, qr/PRIVATE_CGI_PATRON_SENTINEL/, 'the account detail shows its patron' );
    my @rows = disclosure_logs()->as_list;
    is_deeply( [ map { 0 + $_->object } @rows ], [ sort { $a <=> $b } ( $actor->id, $patron->id ) ],
        'the account detail audits the target and its displayed manager' );
    is_deeply( [ map { decode_json( $_->info )->{surface} } @rows ],
        [ ('patrons.account.line_details') x 2 ], 'both subjects share the account-detail surface' );
    ...
    $inject_disclosure_store_failure = 0;
    is( $response->code, 503, 'account detail fails closed on audit failure' );
    unlike( $response->as_string, qr/PRIVATE_CGI_PATRON_SENTINEL/, 'account detail emits no patron data on failure' );
    is( disclosure_logs()->count, 0, 'account detail rolls back all subject rows' );
