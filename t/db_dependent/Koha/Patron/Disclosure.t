...
subtest 'config gates event creation, not an event already in flight' => sub {
    ...
    t::lib::Mocks::mock_config( 'patron_data_disclosure_log', 0 );
    like( $event->commit, qr/\A[0-9a-f-]{36}\z/, 'an in-flight event commits after the config changes' );
...
subtest 'request patron IDs are bounded, canonical, deduplicated, and existence-resolved' => sub {
    $schema->storage->txn_begin;
    t::lib::Mocks::mock_config( 'patron_data_disclosure_log',         1 );
    t::lib::Mocks::mock_config( 'patron_data_disclosure_max_subjects', 3 );
    ...
    t::lib::Mocks::mock_config( 'patron_data_disclosure_max_subjects', 1 );
...
subtest 'exact transactional rows, class union, and event identity' => sub {
    ...
    throws_ok { $event->add_subject( { patron_id => $patron1->id, data_classes => ['contact'] } ) }
        qr/Committed patron disclosure event cannot be changed/,
        'a committed event cannot accept another class or subject';
...
subtest 'a false store result rolls back the complete event' => sub {
    $schema->storage->txn_begin;
    t::lib::Mocks::mock_config( 'patron_data_disclosure_log',         1 );
    t::lib::Mocks::mock_config( 'patron_data_disclosure_max_subjects', 1000 );
    @{ $logger->messages } = ();
    ...
    my $event   = new_event( actor_id => $actor->id );
    $event->add_subject( { patron_id => $patron1->id, data_classes => ['identity'] } );
    $event->add_subject( { patron_id => $patron2->id, data_classes => ['contact'] } );

    my $store       = Koha::ActionLog->can('store');
    my $store_calls = 0;
    my $mock        = Test::MockModule->new('Koha::ActionLog');
    $mock->redefine(
        store => sub {
            my $self = shift;
            return if ++$store_calls == 2;
            return $store->( $self, @_ );
        }
    );

    my $ok = eval { $event->commit; 1 };
    ok( !$ok, 'the false store result is not accepted as a committed event' );
    isa_ok( $@, 'Koha::Exceptions::PatronDisclosure' );
    is( $@->reason, 'storage', 'the failure has a typed safe reason' );
    is( disclosure_logs()->count, 0, 'the earlier row is rolled back' );
    is( scalar @{ $logger->messages }, 0, 'no success message is emitted' );
