...

{
    ...
    sub commit {
        my ($self) = @_;
        $self->{commits}++;
        Koha::Exceptions::PatronDisclosure->throw( reason => 'invalid_subject_limit' ) if $self->{fail};
...
$context_mock->redefine(
    config => sub {
        my ( $class, $name ) = @_;
        return $enabled if $name eq Koha::Patron::Disclosure::CONFIG_ENABLED;
        return;
    },
    preference => sub {
        my ( $class, $name ) = @_;
...
subtest 'an enabled in-flight descriptor commits after the config changes' => sub {
    plan tests => 3;
